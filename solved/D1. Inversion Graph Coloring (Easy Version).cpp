    ...
    forn(i, n) {
        ...
        forn(j, i) {
            ...
            forn(v, n + 1) {
                ...
                if (x >= y) {
                    int best_next_other = -1;
                    bool can_extend_v = (x >= v);
                    
                    if (can_extend_v) {
                        best_next_other = min(v, y);
                    } else {
                        best_next_other = v;
                    }
                    
                    if (best_next_other != -1) {
                        dp[i][best_next_other] = (dp[i][best_next_other] + count) % MOD;
                    }
                } else if (x >= v) {
                    dp[i][y] = (dp[i][y] + count) % MOD;
                }
            }
        }
        
        forn(v, n + 1) {
            total_good = (total_good + dp[i][v]) % MOD;
        }
    }
    out(total_good);
}
