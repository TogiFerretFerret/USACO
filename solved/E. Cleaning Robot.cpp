...
void solve(){
    ...
    matrix<int> dp = make<int>(n + 2, 2);
    matrix<int> dp_b = make<int>(n + 2, 2);
    ...
    f0rn(i, n, 0) {
        ...
        forn(r, 2) {
            bool curr_dirty = (r == 0 ? d0 : d1);
            bool other_dirty = (r == 0 ? d1 : d0);

            int res = dp[i + 1][r];

            if (curr_dirty) {
                res = max(res, 1 + dp[i + 1][r]);
            }

            if (other_dirty) {
                res = max(res, 1 + dp_b[i + 1][1 - r]);
            }

            if (curr_dirty && other_dirty) {
                res = max(res, 2 + dp_b[i + 1][1 - r]);
            }
            dp[i][r] = res;

            int res_b = dp[i + 1][r];
            if (curr_dirty) { 
                res_b = max(res_b, 1 + dp[i + 1][r]);
            }
            dp_b[i][r] = res_b;
        }
    }
